...
var VIS = {
    ...
    getting_started: {                      // special-issue coloring
        exp_interval: 30    // days
    },
...
view_refresh = function (m, v) {
    ...
    if (success) {
        ...
        VIS.annotes.forEach(function (c) {
            ...
            d3.selectAll(c + "-vis").classed("invisible", true);
        });
        ...
        VIS.annotes.forEach(function (c) {
            ...
            d3.selectAll(c + "-vis").classed("invisible", false);
        });
    } else {
...
setup_vis = function (m) {
    ...
    // if the cookie says user hasn't visited lately, show the getting started
    // modal
    if (document.cookie.indexOf('visited=true') === -1) {
        document.cookie = "visited=true; expires=" +
            d3.time.day.offset(new Date(), VIS.getting_started.exp_interval)
                .toUTCString();
        $('#getting_started_modal').modal();
    }
