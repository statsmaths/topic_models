...
var view = function() {
    var that = {},
        ...
        updating, dirty, loading, calculating, error, warning, tooltip, append_weight_tds, plot_svg, append_svg, scroll_top, scroll_origin;
    ...
    error = function(msg) {
        d3.select("div#error").classed("hidden", false).append("p").text(msg);
        this.loading(false);
        VIS.error = true
    };
    that.error = error;
    warning = function(msg) {
        d3.select("div#warning").classed("hidden", false).append("p").text(msg)
    };
    ...
    tooltip = function() {
        ...
        tt.div = d3.select("body").append("div").attr("id", "tooltip").classed("bar_tooltip", true);
    ...
    append_weight_tds = function(sel, f) {
        sel.append("td").classed("weight", true).append("div").classed("proportion", true).style("margin-left", function(w) {
    ...
    plot_svg = function(selector, spec) {
        ...
        if (VIS.svg.has(selector)) {
            ...
            d3.select(selector + " svg").attr("width", spec.w + spec.m.left + spec.m.right).attr("height", spec.h + spec.m.top + spec.m.bottom);
            g.attr("transform", "translate(" + spec.m.left + "," + spec.m.top + ")")
        } else {
    ...
    append_svg = function(selection, spec) {
        return selection.append("svg").attr("width", spec.w + spec.m.left + spec.m.right).attr("height", spec.h + spec.m.top + spec.m.bottom).append("g").attr("transform", "translate(" + spec.m.left + "," + spec.m.top + ")")
    };
...
bib.doc_author = function(doc) {
    var lead, lead_trail, result;
    if (doc.authors.length > 0) {
        lead = doc.authors[0].replace(/,/g, "").split(" ");
        lead_trail = lead.pop();
        if (lead.length >= 2 && lead_trail.search(/^(\d|Jr|Sr|[IXV]+$)/) !== -1) {
        ...
        if (doc.authors.length > 1) {
            if (doc.authors.length > 2) {
                result += ", ";
                result += doc.authors.slice(1, doc.authors.length - 1).join(", ")
            }
...
bib.sort = function(m, major, minor, asc_maj, asc_min) {
    var result = [],
        docs, major_key, minor_key, cmp_maj, cmp_min, cur_major, i, last, get_id = function(d) {
            return d.id
        },
        partition = [];
    if (major === "decade") {
        major_key = function(i) {
            return Math.floor(m.meta(i).date.getUTCFullYear() / 10).toString() + "0s"
        }
    } else if (major === "year") {
        major_key = function(i) {
        ...
        major_key = function(i) {
            return bib.doc_author(m.meta(i)).replace(/^\W*/, "")[0].toUpperCase()
        }
    }
    if (minor === "date") {
        ...
        minor_key = function(i) {
            ...
            result_m += d3.format("05d")(doc.issue === "" ? 0 : doc.issue.replace(/\/.*$/, ""));
    ...
    docs = d3.range(m.n_docs()).map(function(d) {
        return {
            id: d,
            major: major_key(d),
            minor: minor_key(d)
        ...
        return cmp_maj(a.major, b.major) || cmp_min(a.minor, b.minor) || d3.ascending(a.id, b.id)
    });
...
bib.sort.dir = function(p) {
    ...
    if (p.dir === "down") {
        result.major = false;
        if (p.major === "decade" || p.major === "year" || p.major === "issue") {
...
bib.citation = function(doc) {
    var s = bib.doc_author(doc),
        title, mo;
    s = s.replace(/\.?$/, ". ");
    title = doc.title.replace(/“/g, "‘").replace(/”/g, "’").replace(/(^|[-\u2014\/(\[{\u2018\s])"/g, "$1‘").replace(/"/g, "’").replace(/'/g, "’").replace(/ <br><\/br>/g, ". ");
    s += "“" + title + ".”";
    s = s.replace(/’\./g, ".’");
...
model = function(spec) {
    var my = spec || {},
        that = {},
        info, n_docs, has_dt, tw, n, n_top_words, total_tokens, topic_total, alpha, meta, special_issue, vocab, topic_scaled, topic_yearly, valid_year, yearly_total, topic_docs, topic_words, doc_topics, word_topics, year_topics, topic_name, set_dt, set_tw, set_meta, set_topic_scaled;
    my.ready = {};
    ...
    topic_words = function(t, n) {
        ...
        words = this.tw(t).entries();
        words.sort(function(w1, w2) {
            return d3.descending(w1.value, w2.value) || d3.ascending(w1.key, w2.key)
        });
    ...
    word_topics = function(word, n) {
        var t, row, word_wt, n_topics = n || this.n_top_words(),
            result = [],
        ...
        result.sort(function(a, b) {
            return d3.ascending(a.rank, b.rank) || d3.ascending(a.topic, b.topic)
        });
    ...
    year_topics = function(year, n) {
        ...
        for (t = 0; t < this.n(); t += 1) {
            ...
            result.push({
                topic: t,
                weight: series.get(year) || 0
            })
        }
        result.sort(function(a, b) {
            return d3.descending(a.weight, b.weight) || d3.ascending(a.topic, b.topic)
        });
    ...
    set_meta = function(meta_s) {
        ...
        s = meta_s.replace(/^\n*/, "").replace(/\n*$/, "\n");
        my.start_date = new Date(1e3, 0, 1);
        my.end_date = new Date;
...
view.about = function(section) {
    ...
    d3.selectAll("#discussion_text > div").classed("hidden", true);
    elem.classed("hidden", false);
    elem.node().scrollIntoView();
    d3.selectAll("#about_contents li").classed("selected", false);
    d3.select("#about_contents_" + sec).classed("selected", true);
    d3.selectAll("#nav_about li").classed("active", false);
    d3.select("#nav_about_" + sec).classed("active", true);
    return sec
};
"use strict";
view.doc = function(p) {
    ...
    div.select("h2#doc_header").html(bib.citation(p.meta));
    if (p.special) {
        div.select("#doc_remark .special_issue a").attr("href", p.special.url).text(p.special.title)
    }
    div.select("#doc_remark .special_issue").classed("hidden", !p.special);
    div.select("#doc_remark .token_count").text(p.total_tokens);
    div.select("#doc_remark").html(view.doc.uri(p.meta));
    div.select("#doc_remark a.jstor").attr("href", view.doc.uri(p.meta));
    trs = div.select("table#doc_topics tbody").selectAll("tr").data(topics);
    trs.enter().append("tr");
    trs.exit().remove();
    trs.selectAll("td").remove();
    as_t = trs.append("td").append("a").attr("href", function(t) {
        return topic_link(t.topic)
    }).classed("topic_words", true);
    as_t.append("span").classed("name", true).text(function(t, j) {
        return view.topic.label({
            t: t.topic,
            name: p.names[j]
        }).title + ": "
    });
    as_t.append("span").classed("words", true).text(function(t, j) {
    ...
    trs.append("td").classed("td-right", true).text(function(t) {
...
view.model.list = function(p) {
    var trs, divs, token_max, total = d3.sum(p.sums),
        keys, sorter, sort_choice, sort_dir, years = p.yearly[0].keys();
    trs = d3.select("#model_view_list table tbody").selectAll("tr");
    if (!VIS.ready.model_list) {
        trs = trs.data(d3.range(p.yearly.length)).enter().append("tr");
        trs.on("click", function(t) {
        ...
        trs.selectAll("a.topic_words").append("span").classed("name", true);
        trs.selectAll("a.topic_words").append("span").classed("words", true);
        token_max = d3.max(p.sums);
        view.append_weight_tds(trs, function(t) {
            return p.sums[t] / token_max
        });
        trs.append("td").text(function(t) {
            return VIS.percent_format(p.sums[t] / total)
        });
        VIS.ready.model_list = true
    }
    trs.selectAll("td a.topic_words").attr("href", topic_link);
    trs.selectAll("td a.topic_words span.name").text(function(t) {
        return view.topic.label({
            t: t,
            words: p.words[t],
            name: p.names[t]
    ...
    trs.selectAll("td a.topic_words span.words").text(function(t) {
    ...
    sort_dir = p.dir || (sort_choice === VIS.last.model_list.sort ? VIS.last.model_list.dir : "up") || "up";
    ...
    if (sort_dir === "down") {
        sorter = function(a, b) {
            return d3.descending(keys[a], keys[b]) || d3.descending(a, b)
        }
    } else {
        sorter = function(a, b) {
            return d3.ascending(keys[a], keys[b]) || d3.ascending(a, b)
        }
    }
    ...
    d3.selectAll("#model_view_list th.sort").classed("active", function() {
        ...
        d3.select(this).select("a").attr("href", ref)
    }).on("click", function() {
        set_view(d3.select(this).select("a").attr("href").replace(/#/, ""))
    });
    return true
};
"use strict";
view.topic = function(p) {
    ...
    div.select("#topic_subheader").classed("hidden", !ttl.subtitle).select(".topic_subtitle").text(ttl.subtitle || "")
};
view.topic.remark = function(p) {
    d3.select("#topic_view #topic_remark").text(VIS.percent_format(p.col_sum / p.total_tokens) + " of corpus")
};
view.topic.words = function(words) {
    ...
    trs_w = d3.select("table#topic_words tbody").selectAll("tr").data(words);
    ...
    trs_w.append("td").append("a").attr("href", function(w) {
        return "#/word/" + w.word
    }).text(function(w) {
        return w.word
    });
...
view.topic.docs = function(p) {
    var header_text, trs_d, docs = p.docs;
    if (p.year !== undefined) {
        header_text = " in " + p.year;
        d3.select("#topic_year_clear").classed("disabled", false).on("click", function() {
        ...
        header_text = "";
        d3.select("#topic_year_clear").classed("disabled", true)
    }
    d3.selectAll("#topic_docs span.topic_year").text(header_text);
    ...
    trs_d = d3.select("#topic_docs tbody").selectAll("tr").data(docs);
    trs_d.enter().append("tr");
    trs_d.exit().remove();
    trs_d.selectAll("td").remove();
    trs_d.append("td").classed(VIS.special_issue_class, function(d, j) {
        return !!p.specials[j]
    }).append("a").html(function(d, j) {
        return p.citations[j]
    });
    trs_d.on("click", function(d) {
        set_view("/doc/" + d.doc)
    });
    view.append_weight_tds(trs_d, function(d) {
        return d.frac
    });
    trs_d.append("td").classed("td-right", true).text(function(d) {
...
view.topic.yearly = function(p) {
    ...
    spec.w = Math.max(spec.w, VIS.topic_view.w);
    spec.w -= spec.m.left + spec.m.right;
    spec.h = Math.floor(spec.w / VIS.topic_view.aspect) - spec.m.top - spec.m.bottom;
...
view.topic.yearly_barplot = function(param) {
    var series = [],
        scale_x, scale_y, w, w_click, bars, bars_enter, bars_click, axes, tip_text, tx_duration = view.dirty("topic/yearly") ? 1e3 : 0,
        svg = param.svg,
        spec = param.spec;
    series = param.yearly.keys().sort().map(function(y) {
        return [new Date(Date.UTC(+y, 0, 1)), param.yearly.get(y)]
    });
    scale_x = d3.time.scale.utc().domain([series[0][0], d3.time.day.utc.offset(series[series.length - 1][0], spec.bar_width)]).range([0, spec.w]);
    w = scale_x(d3.time.day.utc.offset(series[0][0], spec.bar_width)) - scale_x(series[0][0]);
    w_click = scale_x(d3.time.year.utc.offset(series[0][0], 1)) - scale_x(series[0][0]);
    scale_y = d3.scale.linear().domain([0, d3.max(series, function(d) {
        return d[1]
    })]).range([spec.h, 0]).nice();
    if (param.axes) {
        axes = svg.selectAll("g.axis").data(["x", "y"]);
        axes.enter().append("g").classed("axis", true).classed("x", function(v) {
            return v === "x"
        }).classed("y", function(v) {
            return v === "y"
        }).attr("transform", function(v) {
            return v === "x" ? "translate(0," + spec.h + ")" : "translate(-5, 0)"
        });
        axes.transition().duration(tx_duration).attr("transform", function(v) {
            return v === "x" ? "translate(0," + spec.h + ")" : undefined
        }).each(function(v) {
            var sel = d3.select(this),
                ax = d3.svg.axis().scale(v === "x" ? scale_x : scale_y).orient(v === "x" ? "bottom" : "left");
            if (v === "x") {
                ax = ax.ticks(d3.time.years.utc, spec.ticks)
            } else {
                ax = ax.tickSize(-spec.w).outerTickSize(0).tickFormat(VIS.percent_format).tickPadding(w_click / 2).ticks(spec.ticks)
            }
            sel.call(ax);
            if (v === "y") {
                sel.selectAll("g").filter(function(d) {
                    return d > 0
                }).classed("minor", true)
            }
        })
    }
    bars = svg.selectAll("g.topic_proportion").data(series, function(s) {
        return String(s[0].getUTCFullYear())
    });
    bars_enter = bars.enter().append("g").classed("topic_proportion", true).attr("transform", function(d) {
    ...
    if (param.clickable) {
        ...
        bars_click.transition().duration(tx_duration).attr("x", -w_click / 2).attr("y", 0).attr("width", w_click).attr("height", function(d) {
            return spec.h
        })
    }
    ...
    bars_enter.append("rect").classed("display", true).style("fill", param.color).style("stroke", param.color);
    bars.transition().duration(tx_duration).attr("transform", function(d) {
        return "translate(" + scale_x(d[0]) + ",0)"
    }).select("rect.display").attr("x", -w / 2).attr("y", function(d) {
        return scale_y(d[1])
    ...
    if (param.clickable) {
        ...
        tip_text = function(d) {
            return d[0].getUTCFullYear()
        };
        bars_click.on("mouseover", function(d) {
            var g = d3.select(this.parentNode);
            g.select(".display").classed("hover", true);
            ...
            if (d3.select(this.parentNode).classed("selected_year")) {
                ...
                d3.selectAll(".selected_year").classed("selected_year", false);
...
view.topic.dropdown = function(topics) {
    ...
    lis = d3.select("ul#topic_dropdown").selectAll("li").data(topics, function(t) {
        return t.topic
    });
    lis.enter().append("li").append("a").text(function(t) {
        return view.topic.label(t).title
    }).attr("href", function(t) {
        return topic_link(t.topic)
    });
    lis.sort(function(a, b) {
        return d3.ascending(view.topic.sort_name(a.name), view.topic.sort_name(b.name))
    }).order();
...
view.word = function(p) {
    var div = d3.select("div#word_view"),
        word = p.word,
        n = p.n,
        words, spec, row_height, width, svg, clip, scale_x, scale_y, scale_bar, gs_t, gs_t_enter, gs_t_label, gs_w, gs_w_enter, tx_w;
    d3.select("form#word_view_form").on("submit", function() {
        d3.event.preventDefault();
        var input_word = d3.select("input#word_input").property("value").toLowerCase();
        set_view("/word/" + input_word)
    });
    if (p.word === undefined) {
        return
    }
    div.selectAll("#word_view span.word").text(word);
    ...
    width = Math.max(spec.w, VIS.word_view.w);
    scale_x = d3.scale.linear().domain([0, n]).range([0, width]);
    scale_y = d3.scale.linear().domain([0, Math.max(1, p.topics.length - 1)]).range([row_height, row_height * (Math.max(p.topics.length, 1) + 1)]);
    scale_bar = d3.scale.linear().domain([0, 1]).range([0, row_height / 2]);
    clip = d3.select("#word_view_clip");
    if (clip.size() === 0) {
        clip = d3.select("#word_view svg").append("clipPath").attr("id", "word_view_clip");
        clip.append("rect")
    }
    clip.select("rect").attr("x", -spec.m.left).attr("y", -spec.m.top).attr("width", spec.w + spec.m.left + spec.m.right).attr("height", spec.h + spec.m.top + spec.m.bottom);
    svg.attr("clip-path", "url(#word_view_clip)");
    gs_t = svg.selectAll("g.topic").data(p.topics, function(t) {
        return t.topic
    });
    gs_t.transition().duration(1e3).attr("transform", function(t, i) {
        return "translate(0," + scale_y(i) + ")"
    });
    gs_t_enter = gs_t.enter().append("g").classed("topic", true).attr("transform", function(t, i) {
        return "translate(0," + scale_y(i) + ")"
    }).style("opacity", 0);
    ...
    gs_t.exit().transition().duration(2e3).attr("transform", "translate(0," + row_height * (n + gs_t.exit().size()) + ")").remove();
    gs_t_enter.append("rect").classed("interact", true).attr({
        x: -spec.m.left,
        y: -row_height,
        width: spec.m.left,
        height: row_height
    ...
    gs_t_label = gs_t.selectAll("text.topic").data(function(t, j) {
        ...
        return ws.map(function(w, k) {
            return {
                word: w,
                y: -row_height / 2 - (ws.length / 2 - k - 1) * VIS.word_view.topic_label_leading
            }
        })
    }, function(w, j) {
        return w.word + String(j)
    });
    gs_t_label.enter().append("text").classed("topic", true);
    gs_t_label.exit().remove();
    gs_t_label.attr("x", -VIS.word_view.topic_label_padding).attr("y", function(w) {
        return w.y
    }).text(function(w) {
        return w.word
    ...
    gs_w = gs_t.selectAll("g.word").data(function(t, i) {
        return words[i]
    }, function(d) {
        return d.word
    });
    gs_w_enter = gs_w.enter().append("g").classed("word", true);
    gs_w_enter.append("text").attr("transform", "rotate(-45)");
    gs_w_enter.append("rect").classed("proportion", true).attr({
        x: 0,
        y: 0
    }).attr("width", width / (2 * n)).attr("height", function(d) {
        return scale_bar(d.weight)
    });
    gs_w.selectAll("text").text(function(d) {
        return d.word
    });
    gs_w.selectAll("text, rect").on("click", function(d) {
    ...
    tx_w = gs_w.transition().duration(2e3).attr("transform", function(d, j) {
        return "translate(" + scale_x(j) + ",-" + row_height / 2 + ")"
    }).attr("opacity", 1);
    tx_w.selectAll("text").attr("x", width / (4 * n));
    tx_w.selectAll("rect").attr("width", width / (2 * n)).attr("height", function(d) {
        return scale_bar(d.weight)
    });
    gs_w.exit().transition().delay(1e3).remove();
    VIS.ready.word = true;
    return true
};
"use strict";
view.bib = function(p) {
    ...
    d3.select("a#bib_sort_dir").attr("href", "#/bib/" + p.major + "/" + p.minor + "/" + (p.dir === "up" ? "down" : "up"));
    d3.select("#bib_headings a.top_link").on("click", function() {
        d3.event.preventDefault();
        view.scroll_top()
    });
    lis = d3.select("#bib_view #bib_headings ul").selectAll("li").data(ordering, function(o) {
        return o.key
    });
    ...
    lis.selectAll("a").attr("href", function(o) {
        return "#" + view.bib.id(o.heading)
    }).on("click", function(o) {
        d3.event.preventDefault();
        d3.select("#" + view.bib.id(o.heading)).node().scrollIntoView()
    }).text(function(o) {
        return p.major === "issue" ? view.bib.decode_issue(o.heading, false) : o.heading
    });
    if (p.major === "issue") {
...
view.bib.render = function(p) {
    ...
    view.loading(true);
    sections = d3.select("#bib_main").selectAll("div.section").data(p.ordering, function(o) {
        return o.key
    });
    sec_enter = sections.enter().append("div").classed("section", true).attr("id", function(o) {
        return view.bib.id(o.heading)
    });
    if (p.major === "issue") {
        sec_enter.append("h2").classed(VIS.special_issue_class, function(o) {
            return !!p.specials[o.docs[0]]
        }).html(function(o) {
        ...
        sec_enter.append("h2").html(function(o) {
            return o.heading
        })
    }
    sec_enter.append("ul");
    sections.exit().remove();
    items = sections.select("ul").selectAll("li").data(function(o) {
        return o.docs
    });
...
view.model.yearly = function(p) {
    var spec = {},
        svg, scale_x, scale_y, axis_x, g_axis, area, scale_color, bg, clip, raw, to_plot, paths, labels, render_labels, areas, zoom;
    spec.m = VIS.model_view.yearly.m;
    spec.w = d3.select("#model_view_yearly").node().clientWidth || VIS.model_view.yearly.w;
    spec.w = Math.max(spec.w, VIS.model_view.yearly.w);
    spec.w -= spec.m.left + spec.m.right;
    ...
    bg = svg.selectAll("rect.bg").data([1]).enter().append("rect").classed("bg", true);
    bg.attr("width", spec.w).attr("height", spec.h).classed("bg", true);
    clip = d3.select("#model_yearly_clip");
    if (clip.size() === 0) {
        clip = d3.select("#model_view_yearly svg").append("clipPath").attr("id", "model_yearly_clip");
        clip.append("rect").attr("x", 0).attr("y", 0).attr("width", spec.w).attr("height", spec.h)
    }
    d3.select("#model_yearly_clip rect").transition().duration(2e3).attr("width", spec.w).attr("height", spec.h);
    scale_color = function() {
        var cat20 = d3.scale.category20(),
            seq = [];
        to_plot.order.forEach(function(k, r) {
            seq[k] = r
        });
        return function(t, highlight) {
            var c = cat20(seq[t] % 20);
            return highlight ? d3.hsl(c).brighter(.5).toString() : c
        }
    }();
    scale_x = d3.time.scale.utc().domain(to_plot.domain_x).range([0, spec.w]);
    scale_y = d3.scale.linear().domain(to_plot.domain_y).range([spec.h, 0]).nice();
    axis_x = d3.svg.axis().scale(scale_x).orient("bottom");
    g_axis = svg.selectAll("g.axis").data([1]);
    g_axis.enter().append("g").classed("axis", true).classed("x", true).attr("transform", "translate(0," + spec.h + ")");
    g_axis.transition().duration(2e3).attr("transform", "translate(0," + spec.h + ")").call(axis_x);
    paths = svg.selectAll("path.topic_area").data(to_plot.data);
    paths.enter().append("path").classed("topic_area", true).attr("clip-path", "url(#model_yearly_clip)").style("fill", function(d) {
    ...
    paths.transition().duration(2e3).attr("d", areas);
    labels = svg.selectAll("text.layer_label").data(to_plot.data, function(d) {
        return d.t
    });
    labels.enter().append("text").classed("layer_label", true).attr("clip-path", "url(#model_yearly_clip)");
    labels.exit().remove();
    render_labels = function(sel) {
        var i, j, t, xs, cur, show = [],
            max = [],
            x0 = scale_x.domain()[0],
            x1 = scale_x.domain()[1],
            y0 = scale_y.domain()[0],
        ...
        for (i = 0; i < to_plot.data.length; i += 1) {
            ...
            for (j = 0, cur = 0; j < xs.length; j += 1) {
                if (xs[j].x >= x0 && xs[j].x <= x1 && xs[j].y0 + xs[j].y >= y0 && xs[j].y0 + xs[j].y <= y1) {
                    if (xs[j].y > cur && b - scale_y(xs[j].y) > VIS.model_view.yearly.label_threshold) {
                        show[t] = true;
                        max[t] = j;
                        cur = xs[j].y
                    }
        ...
        sel.filter(function(d) {
            ...
            return scale_y(d.values[max[d.t]].y0 + d.values[max[d.t]].y / 2)
        }).text(function(d) {
            return view.topic.label(d).title
        })
    };
    render_labels(labels.transition().duration(2e3));
    zoom = d3.behavior.zoom().x(scale_x).y(scale_y).scaleExtent([1, 5]).on("zoom", function() {
        if (VIS.zoom_transition) {
            paths.transition().duration(2e3).attr("d", areas);
            svg.select("g.x.axis").transition().duration(2e3).call(axis_x);
            render_labels(labels.transition().duration(2e3));
    ...
    d3.select("button#reset_zoom").on("click", function() {
        ...
        zoom.translate([0, 0]).scale(1).event(svg)
    });
    zoom(svg);
    d3.select("button#yearly_raw_toggle").text(raw ? "Show proportions" : "Show counts").on("click", function() {
        set_view(raw ? "/model/yearly/frac" : "/model/yearly/raw")
    });
    d3.selectAll("#yearly_choice li").classed("active", false);
    d3.select(raw ? "#nav_model_yearly_raw" : "#nav_model_yearly_frac").classed("active", true);
    return true
};
view.model.yearly.stacked_series = function(p) {
    var year_keys, years, all_series, stack, ord, data_frac, data_raw, stack_domain_y;
    if (!VIS.model_view.yearly.data) {
        ...
        ord = stack.order()(all_series.map(function(ds) {
            return ds.values.map(function(d) {
                return [d.x, d.y]
            })
        }));
        ...
        VIS.model_view.yearly.domain_frac = stack_domain_y(data_frac);
        VIS.model_view.yearly.domain_raw = stack_domain_y(data_raw);
...
view.model.plot = function(param) {
    var svg, cloud_size, circle_radius, range_padding, zoom_rect, domain_x, domain_y, scale_x, scale_y, scale_size, scale_stroke, gs, gs_enter, words, translation, zoom, topics = param.topics,
        n = param.topics.length,
        spec = {};
    spec.w = d3.select("#model_view").node().clientWidth || VIS.model_view.w;
    spec.w = Math.max(spec.w, VIS.model_view.w);
    ...
    circle_radius = Math.floor(spec.w / (2.1 * Math.sqrt(VIS.model_view.aspect * n)));
    cloud_size = circle_radius * 1.8;
    range_padding = 1.1 * circle_radius;
    zoom_rect = svg.selectAll("rect.bg").data([1]);
    ...
    if (param.type === "scaled") {
        ...
        topics.forEach(function(p, j) {
            topics[j].sort_key = p.name ? view.topic.sort_name(p.name) : p.words.join(" ")
        });
        topics = topics.sort(function(a, b) {
            return d3.ascending(a.sort_key, b.sort_key)
        });
        view.model.grid_coords(n, VIS.model_view.cols || Math.floor(Math.sqrt(VIS.model_view.aspect * n))).forEach(function(p, j) {
            topics[j].x = p.x;
            topics[j].y = p.y
        })
    }
    domain_x = d3.extent(topics, function(d) {
        return d.x
    });
    domain_y = d3.extent(topics, function(d) {
        return d.y
    });
    scale_x = d3.scale.linear().domain(domain_x).range([range_padding, spec.w - range_padding]);
    scale_y = d3.scale.linear().domain(domain_y).range([spec.h - range_padding, range_padding]);
    scale_size = d3.scale.sqrt().domain([0, 1]).range(VIS.model_view.size_range);
    scale_stroke = d3.scale.linear().domain([0, d3.max(topics, function(p) {
        return p.total
    })]).range([0, VIS.model_view.stroke_range]);
    gs = svg.selectAll("g").data(topics, function(p) {
        return p.t
    });
    gs_enter = gs.enter().append("g");
    gs.exit().remove();
    gs_enter.append("clipPath").attr("id", function(p) {
        return "clip_circ" + p.t
    }).append("circle").attr({
        cx: 0,
        cy: 0
    });
    gs_enter.append("circle").attr("cx", 0).attr("cy", 0).classed("topic_cloud", true).attr("stroke-width", function(p) {
    ...
    words = gs.selectAll("text.topic_word").data(function(p) {
        ...
        for (i = 0; i < wds.length; i += 1, toggle = !toggle) {
            ...
            if (up - cloud_size / 2 < wds[i].size && down > cloud_size / 2) {
                break
            }
        }
        return wds.slice(0, i)
    });
    words.enter().append("text").classed("topic_word", true).attr("clip-path", function(w) {
        return "url(#" + "clip_circ" + w.t
    }).style("font-size", "1px");
    words.text(function(wd) {
        return wd.text
    }).attr("x", 0).attr("y", function(wd) {
        return wd.y
    });
    gs_enter.selectAll("text.topic_name").data(function(p) {
    ...
    }).enter().append("text").classed("topic_name", true).attr("x", 0).attr("y", function(w) {
        return w.y
    }).text(function(w) {
        return w.w
    }).style("font-size", VIS.model_view.name_size + "px").classed("merged_topic_sep", function(w) {
        return w.w === "or"
    });
    ...
    gs.transition().duration(1e3).attr("transform", translation).selectAll("circle").attr("r", circle_radius);
    gs_enter.transition().duration(0).attr("transform", translation).selectAll("circle").attr("r", circle_radius);
    words.transition().duration(1e3).style("font-size", function(wd) {
        return wd.size + "px"
    });
    gs_enter.selectAll("text.topic_word").transition().duration(0).style("font-size", function(wd) {
        return wd.size + "px"
    });
    words.exit().transition().duration(1e3).style("font-size", "1px").remove();
    if (param.type === "scaled") {
        zoom = d3.behavior.zoom().x(scale_x).y(scale_y).scaleExtent([1, 10]).on("zoom", function() {
            if (VIS.zoom_transition) {
                gs.transition().duration(1e3).attr("transform", translation);
                VIS.zoom_transition = false
            } else {
...
view.settings = function(p) {
    ...
    d3.select("#n_words_list").property("min", 1).property("max", p.max_words).property("value", VIS.overview_words).on("change", function() {
        VIS.overview_words = this.valueAsNumber
    });
    d3.select("#n_words_topic").property("min", 1).property("max", p.max_words).property("value", VIS.topic_view.words).on("change", function() {
        VIS.topic_view.words = this.valueAsNumber
    });
    d3.select("#n_topic_docs").property("min", 1).property("max", p.max_docs).property("value", VIS.topic_view.docs).on("change", function() {
        VIS.topic_view.docs = this.valueAsNumber
    });
    d3.select("#highlight_special").property("checked", VIS.special_issue_class === "special_issue").on("change", function() {
    ...
    d3.select("#reveal_hidden").property("checked", VIS.show_hidden_topics === true).on("change", function() {
...
view.words = function(vocab) {
    if (!VIS.ready.words) {
        d3.select("ul#vocab_list").selectAll("li").data(vocab).enter().append("li").append("a").text(function(w) {
            return w
...
var VIS = {
    ...
    bib: {
        keys: {
            major: ["decade", "year", "journal", "issue", "alpha"],
            minor: ["date", "journal", "alpha"]
        },
        author_delimiter: "\t"
    },
    bib_view: {
    ...
    cite_date_format: d3.time.format.utc("%B %Y"),
    uri_proxy: "",
    special_issue_class: "special_issue",
    getting_started: {
        exp_interval: 30
    },
...
var topic_link, topic_hash, topic_view, word_view, words_view, doc_view, bib_view, about_view, model_view, model_view_list, model_view_plot, model_view_yearly, set_view, view_refresh, hide_topics, view_loading, settings_modal, setup_vis, load_data, main;
...
word_view = function(m, w) {
    ...
    if (word) {
        ...
        if (VIS.last.word) {
            word = VIS.last.word;
            div.select("a#last_word").attr("href", "#/word/" + word).text(document.URL.replace(/#.*$/, "") + "#/word/" + word);
...
doc_view = function(m, d) {
    ...
    if (!isFinite(doc) || doc < 0 || doc >= m.n_docs()) {
        ...
        doc = VIS.last.doc;
        div.select("a#last_doc").attr("href", "#/doc/" + doc).text(document.URL.replace(/#.*$/, "") + "#/doc/" + doc);
    ...
    m.doc_topics(doc, m.n(), function(ts) {
        ...
        view.doc({
            ...
            total_tokens: d3.sum(topics, function(t) {
                return t.weight
            }),
            words: topics.map(function(t) {
...
bib_view = function(m, maj, min, dir) {
    ...
    ordering = bib.sort(m, sorting.major, sorting.minor, asc.major, asc.minor);
    if (!VIS.ready.bib) {
...
setup_vis = function(m) {
    if (m.info()) {
        ...
        d3.selectAll(".model_title").html(m.info().title)
    }
    ...
    if (document.cookie.indexOf("visited=true") === -1) {
        document.cookie = "visited=true; expires=" + d3.time.day.offset(new Date, VIS.getting_started.exp_interval).toUTCString();
...
load_data = function(target, callback) {
    ...
    if (document.getElementById(target_id)) {
        return callback(undefined, document.getElementById(target_id).innerHTML)
    }
    if (target.search(/\.zip$/) > 0) {
        return d3.xhr(target).responseType("arraybuffer").get(function(error, response) {
            var zip, text;
